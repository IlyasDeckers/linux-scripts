...
if ! dpkg -s sysbench >/dev/null 2>&1; then
...
sysbench --test=/usr/share/doc/sysbench/tests/db/oltp.lua --db-driver=mysql --oltp-table-size=1000000 --mysql-db=sysbench --mysql-user=root --mysql-password=A0WK51YS --max-time=60 --max-requests=0 --num-threads=${thread} run > /root/SQ-${thread}T-${run}
